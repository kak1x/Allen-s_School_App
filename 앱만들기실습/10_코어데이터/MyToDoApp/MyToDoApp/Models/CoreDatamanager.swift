...
final class CoreDataManager {
    ...
    func getToDoListFromCoreData() -> [ToDoData] {
        ...
        if let context = context {
            ...
            // 정렬순서를 정해서 요청서에 넘겨주기
            let dateOrder = NSSortDescriptor(key: "date", ascending: false) // ascending = 오름차순 -> false = 내림차순
        ...
        if let context = context {
            ...
            if let entity = NSEntityDescription.entity(forEntityName: self.modelName, in: context) {
                ...
                if let toDoData = NSManagedObject(entity: entity, insertInto: context) as? ToDoData {
                    ...
                    toDoData.date = Date() // 날짜를 저장하는 순간의 날짜로 생성
        ...
        if let context = context {
            ...
            do {
                ...
                if let fetchedToDoList = try context.fetch(request) as? [ToDoData] {
                    ...
                    // 임시저장소에서 (요청서를 통해서) 데이터 삭제하기 (delete 메서드)
